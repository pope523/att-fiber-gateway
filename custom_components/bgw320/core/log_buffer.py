"""Circular log buffer for AT&T BGW320 Gateway diagnostics.

Captures INFO+ logs from both the HA adapter
(``custom_components.bgw320``) and Core package
...
def is_monitored_logger(logger_name: str) -> bool:
    """True when *logger_name* belongs to this integration or the engine.

    Diagnostics fallbacks filter arbitrary log records down to ours. The
    engine keeps its upstream package name
    (``solentlabs.cable_modem_monitor_core``) even though the HA adapter
    is ``custom_components.bgw320``, so a single substring test on the
    domain would silently drop every engine record.
    """
    return any(logger_name.startswith(monitored) for monitored in _MONITORED_LOGGERS)
...
def setup_log_buffer(hass: HomeAssistant) -> None:
    ...
    (``custom_components.bgw320``) and the Core package
