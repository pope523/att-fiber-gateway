"""Diagnostics support for the AT&T BGW320 Gateway integration.
...
def _get_logs_from_system_log_handler(
    ...
    for record in handler.records:
        if is_monitored_logger(str(record.name)):
            logs.append(
                _create_log_entry(
...
def _parse_legacy_record(record: Any) -> dict[str, Any] | None:
    ...
    try:
        if hasattr(record, "name") and hasattr(record, "message"):
            if not is_monitored_logger(str(record.name)):
                return None
        ...
        if isinstance(record, tuple) and len(record) >= 4:
            ...
            if not is_monitored_logger(str(logger_name)):
                return None
...
def _build_diagnostics_dict(
    ...
    diagnostics: dict[str, Any] = {
        "_solentlabs": {
            "tool": "bgw320/diagnostics",
            "version": VERSION,
            "captured_at": datetime.now().isoformat(),
            "note": ("Captured with AT&T BGW320 Gateway diagnostics"),
        },
