...
Storage keys are namespaced `bgw320.{entry_id}.{domain}`
...
All HA-side recovery wiring lives in `custom_components/bgw320/recovery_adapter.py`.
...
Developer Tools → Events → enter `bgw320_data_updated` →
...
`bgw320`). Falls back to all loaded entries when no
device is specified.
...
automation:
  trigger:
    - platform: state
      ...
      for: "00:01:00"
  action:
    - service: bgw320.request_refresh
      data:
...
automation:
  trigger:
    - platform: state
      ...
      to: "off"
  action:
    - service: bgw320.request_health_check
      data:
        device_id: <modem_device_id>
    - delay: "00:00:30"
    - service: bgw320.request_refresh
...
|---------|---------------|-----------------|
| Onboarding | First successful poll after setup | `bgw320_onboarding_{entry_id}` |
| Change | Downstream or upstream total differs from the persisted baseline | `bgw320_channel_change_{entry_id}` |
