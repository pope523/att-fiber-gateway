...
[#161](https://github.com/pope523/att-fiber-gateway/discussions/161)
...
[#144](https://github.com/pope523/att-fiber-gateway/issues/144) is the
...
- Files at repo-root `docs/examples/` are outside the working directory of
  ...
  `custom_components/bgw320/` per
  `.github/workflows/release.yml` lines 80-88:

  ```bash
  cd custom_components/bgw320
  # Exclude docs/ (dev specs, not needed at runtime) and
  # any bytecode cache.
  zip -r ../../bgw320.zip . -x 'docs/*' '*__pycache__*'
  ```
...
1. User finds the blueprint URL via CMM docs or community post. The canonical
   ...
   ```text
   https://github.com/pope523/att-fiber-gateway/raw/main/docs/examples/blueprints/<type>/<name>.yaml
   ```
...
- **Static Lovelace dashboards via HACS dashboard category.** Separate
  ...
  `bgw320.generate_dashboard` service (implemented in
...
| Issue [#144](https://github.com/pope523/att-fiber-gateway/issues/144) | Feature request that motivated this spec |
| Discussion [#161](https://github.com/pope523/att-fiber-gateway/discussions/161) | First concrete blueprint candidate (signal_health) |
