...
|------|----------|----------------|---------|
| `number` | `channel_number` (modem's row position) | `sensor.bgw320_ds_ch_1_power` | Yes (new installs) |
| `id` | `(channel_type, channel_id)` (CMTS assignment) | `sensor.bgw320_ds_qam_ch_29_power` | No |
...
| `model` | "BGW320 {model}" | Multi-modem by model |
| `ip` | "BGW320 {host}" | Multi-modem by IP |

All options produce entity_ids prefixed with `bgw320_`
(e.g., `sensor.bgw320_status`). The `model` and `ip` options
...
All unique_ids are prefixed with `{entry_id}_bgw320_`:

```text
{entry_id}_bgw320_{suffix}
```
...
```text
{entry_id}_bgw320_{ds|us}_ch_{channel_number}_{field}
```

Example: `abc123_bgw320_ds_ch_1_power`

ID mode (`id`):

```text
{entry_id}_bgw320_{ds|us}_{channel_type}_ch_{channel_id}_{field}
```

Example: `abc123_bgw320_ds_ofdm_ch_159_power`

**LAN sensors:**

```text
{entry_id}_bgw320_lan_{interface}_{metric}
```
