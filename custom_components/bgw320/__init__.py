"""The AT&T BGW320 Gateway integration.

Home Assistant adapter layer for monitoring the AT&T Nokia BGW320-505
XGS-PON fiber gateway — optical Rx/Tx power, temperature, PON link
state, WAN status and counters, plus a nonce-authenticated restart.
Consumes Core (orchestration, parsing, auth) and Catalog (device
config, parser) packages — all device-specific logic lives there.
...
async def async_setup_entry(
    ...
    """Set up AT&T BGW320 Gateway from a config entry.
    ...
    _LOGGER.info(
        "AT&T BGW320 Gateway v%s starting [%s %s] — %s",
        VERSION,
    ...
    data_coordinator = DataUpdateCoordinator[ModemSnapshot](
        hass,
        _LOGGER,
        name=f"BGW320 {coordinator_label}",
    ...
    if health_monitor is not None:
        ...
        health_coordinator = DataUpdateCoordinator[HealthInfo](
            hass,
            _LOGGER,
            name=f"BGW320 {coordinator_label} Health",
