"""Utility functions for the AT&T BGW320 Gateway integration."""
...
def get_device_name(entity_prefix: str, *, model: str = "", host: str = "") -> str:
    ...
    """
    # The device name is slugified into every entity_id, so the default is
    # kept short ("BGW320") to yield sensor.bgw320_* rather than a verbose
    # "AT&T BGW320 Gateway" prefix, which slugifies to at_t_bgw320_gateway_*.
    if entity_prefix == EntityPrefix.MODEL:
        return model or "BGW320"
    if entity_prefix == EntityPrefix.IP:
        return f"BGW320 {host}"
    return "BGW320"
